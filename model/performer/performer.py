...
def orthogonal_matrix_chunk(cols, device = None):
    ...
    q, r = torch.linalg.qr(unstructured_block.cpu(), 'reduced' if (some:=True) else 'complete')
...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        if output_attentions:
            ...
            #attn_weights = torch.zeros(q.shape[0], 1, q.shape[2], q.shape[2]).to(device).to(torch.float16) #q.shape[0] is batch size
            head_attns = []
            for head_dim in range(q.shape[1]):
                ...
                head_attns.append(torch.abs(attn_fn(q[:,head_dim].to(torch.float16), k[:,head_dim].to(torch.float16), v_diag[:,head_dim].to(torch.float16))))
            ...
            attn_weights = torch.mean(attn_weights, dim=1, keepdim=True)
...
def filter_gene2vec_weight(path, gene_list, dim=200):

    if not os.path.exists(path):
        raise FileNotFoundError(f"Embedding file not found at: {path}")
    
    filtered_embeddings = {}
    
    gene_set = set(gene_list)  # Convert list to set for O(1) lookups
    print("Filtering gene2vec embeddings...")
    # Get line count for tqdm without loading the whole file into memory
    with open(path, 'r', encoding='utf-8') as f:
        num_lines = sum(1 for _ in f)

    with open(path, 'r', encoding='utf-8') as f:     
        # Process each subsequent line
        for line in tqdm(f, desc="Filtering gene2vec embeddings", total=num_lines):
            line = line.strip()
            if not line:
            ...
            parts = line.split()
            if len(parts) < dim + 1:
                continue  # Skip invalid lines (not enough elements)
            
            gene = parts[0]
            try:
                vector = [float(x) for x in parts[1:]]
            except ValueError:
                continue  # Skip lines with non-numeric vector elements
            
            if gene in gene_set:
                filtered_embeddings[gene] = vector

    unseen_genes = gene_set - set(filtered_embeddings.keys())
    for gene in tqdm(unseen_genes, desc="Initializing unseen genes"):
        filtered_embeddings[gene] = np.random.randn(dim).tolist()
    print(f"Filtered embeddings for {len(filtered_embeddings)} genes. {len(unseen_genes)} genes were not found and initialized to random vectors.")
    return filtered_embeddings


def load_filtered_embeddings(filtered_embeddings, gene_list, dim=200):
    '''
    Expects filtered_embeddings to match size with gene_list
    '''
    assert len(filtered_embeddings) == len(gene_list), "Filtered embeddings size must match gene_list size"
    if not gene_list:
    ...
    for gene in gene_list:
        vector = filtered_embeddings[gene]
        if len(vector) != dim:
            raise ValueError(f"Inconsistent embedding dimension for gene {gene}: expected {dim}, got {len(vector)}")
...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len, gene_list, embeddings=None, freeze=True):
        super().__init__()
        if embeddings is not None:
            gene2vec_weight = load_filtered_embeddings(embeddings, gene_list, dim)
        else:
            print("No gene2vec embeddings provided, initializing randomly.")
            gene2vec_weight = np.random.randn(len(gene_list), dim)

        # Add a zero vector for the [CLS] token or other special tokens
        ...
        self.emb = nn.Embedding.from_pretrained(gene2vec_weight, freeze=freeze)
...
def read_gene_list(gene_csv):
    gene_list = pd.read_csv(gene_csv)['gene_name'].tolist()
...
class PerformerLM(nn.Module):
    def __init__(
        self,
        *,
        gene_list,                          # list of genes for gene2vec positional embedding
        ...
        self.g2v_embeddings = g2v_embeddings  # for checkpoint loading 

        if g2v_embeddings is not None:
            self.pos_emb = Gene2VecPositionalEmbedding(dim, max_seq_len, gene_list=gene_list, embeddings=g2v_embeddings)
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        if hasattr(self, 'inference_gene_list') and self.inference_gene_list is not None:
            # Pad the input tensor if it's for a subset of genes
            pad_token_id = self.num_tokens - 1
            padded_x = torch.full((b, self.max_seq_len), pad_token_id, device=device, dtype=torch.long)

            training_gene_indices = {gene: i for i, gene in enumerate(self.gene_list)}
            inference_indices_in_training = [training_gene_indices[gene] for gene in self.inference_gene_list]

            # Place gene values from input into correct positions, assuming special token is last
            padded_x[:, inference_indices_in_training] = x[:, :-1]
            # Copy special token to the end
            padded_x[:, -1] = x[:, -1]
        ...
        if output_attentions:
            ...
            #Manu: this is of dimension (batch_size, sequence_length, embedding_dimension)
    ...
    @classmethod
    def from_checkpoint(cls, ckpt_path, n_genes, gene_csv, g2v_embeddings='model/gene2vec/gene2vec_80.txt', device='cpu', inference_gene_list=None):

        def get_num_tokens(model_weights):
            return model_weights['token_emb.weight'].shape[0]
        
        def get_dim_embedding(model_weights):
            return model_weights['token_emb.weight'].shape[1]
        
        def get_n_layers(model_weights):
            return max([ int(re.match("performer.net.layers.(.*).0.", x).group(1)) for x in model_weights if re.match("performer.net.layers.(.*).0.", x) ]) + 1 

        ckpt = torch.load(ckpt_path, map_location=device)
        model_weights = { k.replace("_orig_mod.", ""): v for k,v in ckpt['model_state_dict'].items() }
    
        gene_list = read_gene_list(gene_csv)
        embeddings = filter_gene2vec_weight(g2v_embeddings, gene_list, dim=200)
        
        model = PerformerLM(**{
            "num_tokens": get_num_tokens(model_weights),
            "dim": get_dim_embedding(model_weights),
            "depth": get_n_layers(model_weights),
            "max_seq_len": n_genes + 1,
            "heads": 10,
            "local_attn_heads": 0,
            "g2v_embeddings": embeddings,
            "gene_list": gene_list,
        }).to(device)
        
        model.load_state_dict(model_weights)

        if inference_gene_list is not None:
            assert all(g in model.gene_list for g in inference_gene_list), "All inference genes must be present in the model's original gene list."
            model.inference_gene_list = inference_gene_list
        else:
            model.inference_gene_list = None
